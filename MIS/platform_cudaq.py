"""
platform_cudaq.py  —  CUDA-Q worker for MIS
...
if not EDGES:
    print(json.dumps({"status": "ERROR", "error": "Missing edges in config", "platform": "CUDA-Q", "p": P, "n": N}))
    sys.exit(1)
...
def objective(params: np.ndarray) -> float:
    ...
    return -sum(score([int(b) for b in bs.zfill(N)]) * cnt for bs, cnt in counts.items()) / SHOTS
...
mean_score = sum(score([int(b) for b in bs.zfill(N)]) * cnt for bs, cnt in counts.items()) / total
mean_size = sum(independent_size([int(b) for b in bs.zfill(N)]) * cnt for bs, cnt in counts.items()) / total
best_size = max(independent_size([int(b) for b in bs.zfill(N)]) for bs in counts)
feasible = sum(cnt for bs, cnt in counts.items() if conflicts([int(b) for b in bs.zfill(N)]) == 0) / total
